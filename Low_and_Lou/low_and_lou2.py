...
def B_r_temp(r):
    theta = np.pi / 2
    mu = np.cos(theta)
    return r**2 * B_r(r, theta)

def B_theta_temp(theta):
    r = 1
    mu = np.cos(theta)
    return B_theta(r, theta) * np.sin(theta)
...
print(np.max(np.abs(divb)))
print(np.mean(np.abs(divb)) * dx)
print(np.min(bbz[:, :, 0]))
...
# fig = plt.figure()
# ax = fig.add_subplot(111)
# im = ax.imshow(B_z(x_grid, y_grid, z_min)  * b_norm, \
#                extent = [x_min, x_max, y_min, y_max], \
#                origin = 'lower')
# cb = fig.colorbar(im)
...
# start_points[:, 1] = z_min
# y_grid, z_grid = np.meshgrid(y, z)
# fig = plt.figure()
# ax = fig.add_subplot(111)
# ct = ax.streamplot(y_grid, z_grid, B_y(0.2, y_grid, z_grid), B_z(0.2, y_grid, z_grid), \
